...
%***********LINE 516 MODIFED FOR ALLAN VARIANCE**************
...
GyT = dataT(:, 3);
GyRawData = transpose(GyT);
GzT = dataT(:, 4);
GzRawData = transpose(GzT);

% TimestampT = dataT(:, 5);
% Timestamp = transpose(TimestampT);
...
thetaX = cumsum(Gx,1)*t0;
DataX.freq = Gx;
DataX.rate = Fs;
thetaY = cumsum(Gy,1)*t0;
DataY.freq = Gy;
DataY.rate = Fs;
thetaZ = cumsum(Gz,1)*t0;
DataZ.freq = Gz;
DataZ.rate = Fs;

OAVARX = allan_overlap(DataX);
OAVARY = allan_overlap(DataY);
OAVARZ = allan_overlap(DataZ);
asdf=1;
...
if verbose >= 1 % show analysis results
    ...
    if ~isempty(sm)
        ...
        loglog(tau,power(sm,2),'.-b','LineWidth',plotlinewidth,'MarkerSize',24);
        ...
        grid on;
        title(['Overlapping Allan Variance: ' name],'FontSize',FontSize+2,'FontName',FontName);
        %set(get(gca,'Title'),'Interpreter','none');
