...
namespace UnscentedKalmanFilter
{
    public class UKF
    {
        ...
        private void init()
        {
            double[,] array2D = new double[,] { { 1, 2 ,3}, { 3, 4,6}, { 5, 6,4}, { 7, 8,9 } };
          
            Test = Matrix.Build.DenseOfArray(array2D);
        
            Test2 = Matrix.Build.Dense(6, 7);
        ...
        public void printMatrix(Matrix<double> M, String S)
        {
            System.Diagnostics.Debug.WriteLine("\nArray:" + S + "\n");
            System.Diagnostics.Debug.WriteLine("RowCount:{0}, ColumnCount:{1}" , M.RowCount ,M.ColumnCount + "\n");
            double[,] MArray = new double[(M.RowCount), (M.ColumnCount)];
            MArray = M.ToArray();
            for (int i = 0; i < M.RowCount; i++)
            {           
                for (int j = 0; j < M.ColumnCount; j++)
                {
                    System.Diagnostics.Debug.Write(MArray[i, j] + ",");
                }
                System.Diagnostics.Debug.WriteLine("\n");
            }
        }
